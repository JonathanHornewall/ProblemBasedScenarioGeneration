...
const CONFIG_VERSION = "temp-dfl-suite-power-sweep-v1"
...
const BASELINE_DEMAND_POWER = 2.0
const DEMAND_POWERS = [1.0, 0.5, 1.5]
...
function power_path_part(power)
    text = replace(string(Float64(power)), "." => "p", "-" => "m")
    return "p_" * text
end
...
function generate_resource_allocation_dataset(;
    ...
    demand_power=BASELINE_DEMAND_POWER,
)
...
function generate_training_dataset(config)
    dataset = generate_resource_allocation_dataset(
        ...
        demand_power=Float64(config.demand_power),
    )
...
function test_cache_paths(; smoke=false, demand_power=BASELINE_DEMAND_POWER)
    dir = artifact_path(power_path_part(demand_power), smoke ? "smoke_test_cache" : "test_cache")
    return (;
        dir=dir,
...
function test_cache_exists(; smoke=false, demand_power=BASELINE_DEMAND_POWER)
    paths = test_cache_paths(smoke=smoke, demand_power=demand_power)
    return isfile(paths.dataset) && isfile(paths.optimal_results) && isfile(paths.metadata_jls)
end

function load_test_cache(; smoke=false, demand_power=BASELINE_DEMAND_POWER)
    paths = test_cache_paths(smoke=smoke, demand_power=demand_power)
    test_cache_exists(smoke=smoke, demand_power=demand_power) ||
...
function ensure_test_cache!(; smoke=false, force=false, demand_power=BASELINE_DEMAND_POWER)
    paths = test_cache_paths(smoke=smoke, demand_power=demand_power)
    if !force && test_cache_exists(smoke=smoke, demand_power=demand_power)
        return load_test_cache(smoke=smoke, demand_power=demand_power)
    end
    ...
    dataset = generate_resource_allocation_dataset(
        ...
        demand_power=Float64(demand_power),
    )
    objects = problem_objects()
