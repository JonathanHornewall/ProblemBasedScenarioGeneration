...
function run_experiment()
    ...
    ground_truth_dir = get(ENV, "CDFL_SANDBOX_GROUND_TRUTH_DIR", joinpath(SANDBOX_DIR, "ground_truth"))
    mkpath(ground_truth_dir)
    data_path = joinpath(ground_truth_dir, "resource_allocation_data_seed_$(seed).jls")

    if isfile(data_path)
        saved_data = Serialization.deserialize(data_path)
        data_set_training = saved_data.data_set_training
        data_set_testing = saved_data.data_set_testing
        println("Loaded shared sandbox data from: $(data_path)")
    else
        ...
        println("Saved shared sandbox data to: $(data_path)")
    end

    if env_bool("CDFL_SANDBOX_ONLY_PREPARE_DATA", false)
        println("Prepared shared sandbox data only.")
        return
    end
    ...
    default_epochs = env_int("CDFL_SANDBOX_DEFAULT_EPOCHS", 50)
    first_stage_epochs = env_int("CDFL_SANDBOX_FIRST_STAGE_EPOCHS", 50)
    ...
    display_iterations = env_bool("CDFL_SANDBOX_DISPLAY_ITERATIONS", false)
    display_smooth = env_bool("CDFL_SANDBOX_DISPLAY_SMOOTH", false)
    ...
    run_final_stage = env_bool("CDFL_SANDBOX_RUN_FINAL_STAGE", false)
    ...
    if skip_training
        ...
        if !run_final_stage
            println("Skipping final annealing stage because CDFL_SANDBOX_RUN_FINAL_STAGE is not set.")
        elseif final_stage_index in completed_stage_numbers
