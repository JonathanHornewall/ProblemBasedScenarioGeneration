...
z^{\#,x}\in\argmin_{z\in Z}G(z,x)\qquad \text{(obtained via interior point method)}.
\end{equation}
We will also consider the optimal solution for single-scenario problems:
\begin{equation}
\label{eq:canonical-single-scenario}
\hat z^{\#}(\xi)\in\argmin_{z\in Z}\hat G(z,\xi).
\end{equation}
...
\end{equation}
By classical interior point theory, as the smoothing parameter approaches zero, the solutions of
the smoothed problem approach the solution to the original problem:
\begin{equation}
