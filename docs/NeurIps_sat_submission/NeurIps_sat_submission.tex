...
\begin{abstract}
This paper studies decision focused scenario learning for contextual two-stage stochastic programming. The approach has emerged as a promising research direction, but challenges exist. Existing methods typically require specific assumptions on which second stage is random. Currently, a general theory for when scenario policies can theoretically yield optimal decision policies is also lacking. Generally, the non-differentiability of the decision-focused objective is a source of difficulty. This paper make three contributions. We first present a comprehensive theory for when scenario learning can encode optimal policies. It additionally shows an equivalence result between learning different second-stage data, significantly extending existing methods by relaxing required assumptions on second stage randomness. Secondly, we study log-barrier smoothing, showing how the it induces smoothnes and invexity in the decision focused loss. Finally, we present a novel method inpsired by interior point methods from classical optimization. It accommodates randomness in arbitrary second stage data, learns directly on the downstream decision objective without the need for surrogates, and has a natural selection rule for when optima are non-unique. We demonstrate its effectiveness on standard benchmarks.
\end{abstract}
