...
const EXPERIMENT_ID = "transshipment/experiment_1_tiny"
const EXPERIMENT_NAME = "transshipment_experiment_1_tiny"
...
experiment_module_name() = :TransShipmentExperiment1Tiny
...
scenario_decoder(problem_instance=problem()) =
    ContextualDFLExperiments.TransShipmentPositiveHVectorDecoder(problem_instance)
...
function learned_parameter_count(problem_instance)
    ...
    return length(mean_parameters.rhs)
end
...
function target_from_scenario(scenario::ContextualDFL.ParametricScenario)
    isempty(scenario.h_eq_xi) &&
        throw(ArgumentError("expected transshipment RHS values in h_eq_xi"))
